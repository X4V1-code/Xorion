...
void CommandMgr::initCommands() {
    ...
    // Alphabetical list (by class name)
...
void CommandMgr::execute(char* message) {
    ...
    for (auto it = this->commandList.begin(); it != this->commandList.end(); ++it) {
        ...
        if (commandCopy.find(cmd) != std::string::npos) // partial match suggestion
            Game.getGuiData()->displayClientMessageF("%sDid you mean to type: %s?", RED, commandCopy.c_str());
    }
}
