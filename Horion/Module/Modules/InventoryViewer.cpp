...
void InventoryViewer::drawInventory(Entity* ent) {
    if (!ent) return;
    ...
    for (int row = 0; row < 4; row++) {
        for (int col = 0; col < 9; col++) {
            ...
            DrawUtils::fillRectangle(Vec4(p.x, p.y, p.x + cell - 2, p.y + cell - 2), slotCol, 1.0f);
        }
    }
    ...
    vec2_t offset;
    offset.x = 0;
    offset.y = -14.0f * scale;
    DrawUtils::drawText(base.add(offset), &name, MC_Color(255, 255, 255), scale);
}
