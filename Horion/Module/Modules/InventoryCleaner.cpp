...
void InventoryCleaner::onTick(GameMode* gm) {
	if (false && openInv) // TODO: g_Data.getLocalPlayer()->canOpenContainerScreen() && openInv 
		return;
	...
	if (!dropSlots.empty()) {
		for (int i : dropSlots) {
			// TODO: g_Data.getLocalPlayer()->getSupplies()->inventory->dropSlot(i);
		}
	}
	...
	if (autoSort) {
		// Put sword in first slot
		{
			...
			if (item != 0) {} // TODO: inv->moveItem(item, 0);
		}
	}
}
...
std::vector<int> InventoryCleaner::findStackableItems() {
	...
	for (int i = 0; i < 36; i++) {
		ItemStack* itemStack = g_Data.getLocalPlayer()->getSupplies()->inventory->getByGlobalIndex(i);
		if (itemStack->item != nullptr) {
			if (itemStack->getItem()->getMaxStackSize(0) > itemStack->count) {
				for (int i2 = 0; i2 < 36; i2++) {
					if (i2 == i) continue;
					ItemStack* itemStack2 = g_Data.getLocalPlayer()->getSupplies()->inventory->getByGlobalIndex(i2);
					if (itemStack2->getItem()->getMaxStackSize(0) > itemStack->count) {
...
std::vector<int> InventoryCleaner::findUselessItems() {
...

	{
		for (int i = 0; i < 36; i++) {
			...
			if (itemStack->item != nullptr) {
				...
				if (itemStack->getItem()->itemId == 261 && !isLastItem(itemStack->getItem()))
					uselessItems.push_back(i);
					else
						items.push_back(itemStack);	
				}
	...
	// Filter armor
	{
		...
		for (int i = 0; i < 36; i++) {
			...
			ItemStack* itemStack = g_Data.getLocalPlayer()->getSupplies()->inventory->getByGlobalIndex(i);
			if (itemStack->item != nullptr && itemStack->getItem()->isArmor()) {
				ArmorItem* armor = reinterpret_cast<ArmorItem*>(itemStack->getItem());
				if (armor->isHelmet()) {
...
bool InventoryCleaner::isLastItem(Item* item) {
	...
	for (int i = 0; i < 36; i++) {
		ItemStack* stack = g_Data.getLocalPlayer()->getSupplies()->inventory->getByGlobalIndex(i);
